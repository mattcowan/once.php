    ...
    if (!$isE2E && trim($content) === '') {
        ...
        if (strlen($passcode) !== 6 || !ctype_alnum($passcode)) {
            ...
            if (!saveNote($path, $note)) {
                ...
                if ($isE2E) {
                    echo '<p>Send this full link to the recipient &mdash; it carries the decryption key, and without the complete link the note can never be opened, even by you:</p>';
...
echo '<p style="font-size:.85rem;color:var(--text-muted);margin:.25rem 0 .75rem 1.6rem">When E2E is on, the decryption key is placed in the share link itself — never sent to the server. You must send the recipient the full link; the passcode alone will not unlock the note.</p>';
...
echo '<ul>';
echo '<li><strong>Read once, then gone.</strong> The note self-destructs after a single view, and expires in 72 hours if never opened.</li>';
echo '<li><strong>End-to-end encryption (E2E):</strong> your browser encrypts the note and keeps the key in the share link. The server only ever stores scrambled text — no one with access to the server (including us) can read it.</li>';
echo '<li><strong>Server-side encryption</strong> (E2E unchecked): the server encrypts and decrypts the note for you. Use this if the recipient cannot run JavaScript.</li>';
echo '<li><strong>Always share two things:</strong> the link <em>and</em> the 6-character passcode — ideally over separate channels (e.g. link by email, passcode by text).</li>';
echo '</ul>';
echo '</div>';
echo '<script nonce="' . html(cspNonce()) . '">
(function(){
